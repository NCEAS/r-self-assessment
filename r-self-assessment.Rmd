...
_WARNING: The order that packages are loaded in matters! For instance, both the`dplyr` and `MASS` packages define a function called `select()`, which do very different things. If both are loaded in via the `library()` function, `R` will use the `select()` function from the more recent `library()` call, unless we specify using `<package_name>::`._
...
Which package will the function `select()` be called from when the packages are loaded in the following order:
